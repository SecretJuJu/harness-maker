...
- **Mechanism selection flow** (SKILL.md §3): classify rule first, producer second. Task-scoped procedure (how-to for a kind of work) → **Skill**; always-on code constraint → lint tool; repo/git state rule → git hook; semantic judgment → Claude Code `prompt` hook; codebase exploration → `agent` hook; Claude-facing reinforcement for any of the above → `.claude/rules/`. Pre-commit is a backstop, **never a default**. "Who violates (Claude vs human)" is answered *after* the primary mechanism is chosen, only to decide whether to add a backstop. Changes to this taxonomy ripple through every doc.
